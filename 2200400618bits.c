...
{
    int byte_x = (x >> (n << 3)) & 0xFF;
    int byte_y = (y >> (n << 3)) & 0xFF;
...
int mul2OK(int x)
{
    int sign_bit = (1 << 16) << 15;  // 1 << 31
    int overflow_bit = (1 << 16) << 14;  // 1 << 30
    return ~((x & sign_bit) | (x & overflow_bit)) & 1;
}
/*
...
{
    int mult3 = (x << 1) + x;
    int round_up = (mult3 >> 31) & 1;
    return (mult3 + round_up) >> 1;
}
/* 
...
int subOK(int x, int y)
{
    ...
    return ~((x_sign ^ y_sign) & (x_sign ^ diff_sign)) & 1;
}
/* 
